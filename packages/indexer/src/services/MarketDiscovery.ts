...
export class MarketDiscovery {
  ...
  async discover(): Promise<DiscoveredMarket[]> {
    ...
    if (failedPrograms === programIds.length && programIds.length > 0) {
      ...
      const err = new Error(
        `Market discovery failed for all ${programIds.length} programs (consecutive: ${this.consecutiveFailures})`,
      );
  ...
  async start(intervalMs = 300_000) {
    ...
    for (let attempt = 0; attempt <= INITIAL_RETRY_DELAYS.length; attempt++) {
      try {
        ...
        if (attempt < INITIAL_RETRY_DELAYS.length) {
          ...
          logger.warn(`Initial discovery returned 0 markets, retrying in ${delay / 1000}s`, {
            attempt: attempt + 1,
          });
        ...
        captureException(err, {
          tags: { context: 'market-discovery-initial', attempt: String(attempt + 1) },
        });
        if (attempt < INITIAL_RETRY_DELAYS.length) {
          const delay = INITIAL_RETRY_DELAYS[attempt];
    ...
    if (!initialSuccess) {
      logger.error(
        'Initial market discovery exhausted all retries — will continue with periodic polling',
      );
    }
