...
describe('logHelpers', () => {
  ...
  describe('logOperation', () => {
    it('logs operation start with info level', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('market-discovery: Discovering markets on Mainnet', {
    ...
    it('includes context in operation log', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('liquidation-scan: Scanning for liquidations', {
  ...
  describe('logTransientError', () => {
    it('logs transient errors with warn level', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('fetchSlab: Attempt 1/3 failed, retrying in 500ms', {
        operation: 'fetchSlab',
        attempt: 1,
        maxAttempts: 3,
    ...
    it('includes context when retry fails', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('rpc-call: Attempt 2/5 failed, retrying in 1000ms', {
        operation: 'rpc-call',
        attempt: 2,
        maxAttempts: 5,
