  ...
  app.get('/markets', async (c) => {
    const result = await withDbCacheFallback(
      ...
      async () => {
        ...
        const { data, error } = await getSupabase().from('markets_with_stats').select('*');
        ...
        return (data ?? [])
          ...
          .map((m) => ({
            ...
            lastPrice:
              m.last_price != null &&
              Number.isFinite(m.last_price) &&
              m.last_price > 0 &&
              m.last_price <= 1_000_000
                ? m.last_price
                : null,
            ...
            markPrice:
              m.mark_price != null &&
              Number.isFinite(m.mark_price) &&
              m.mark_price > 0 &&
              m.mark_price <= 1_000_000
                ? m.mark_price
            ...
            indexPrice:
              m.index_price != null &&
              Number.isFinite(m.index_price) &&
              m.index_price > 0 &&
              m.index_price <= 1_000_000
                ? m.index_price
                : null,
            fundingRate:
              m.funding_rate != null &&
              Number.isFinite(m.funding_rate) &&
              Math.abs(m.funding_rate) <= 10_000
                ? m.funding_rate
                : null,
